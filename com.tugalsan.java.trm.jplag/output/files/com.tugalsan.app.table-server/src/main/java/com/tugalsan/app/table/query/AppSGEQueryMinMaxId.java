...
public class AppSGEQueryMinMaxId extends TS_SGWTExecutor {

    final private static TS_Log d = TS_Log.of(AppSGEQueryMinMaxId.class);
    ...
    public TS_SGWTValidationResult validate(TS_ThreadSyncTrigger servletKillTrigger, HttpServletRequest rq, TGS_SGWTFuncBase funcBase) {
        var f = (AppSGFQueryMinMaxId) funcBase;
...
//        var loginCard = (TGS_LibLoginCard) vldRtn;
        ...
        var sqlMin = anchor.tagSelectAndSpace() + "MIN(" + f.getInput_columnName() + ") FROM " + f.getInput_tableName();
        var sqlMax = anchor.tagSelectAndSpace() + "MAX(" + f.getInput_columnName() + ") FROM " + f.getInput_tableName();
        var where = TGS_StringUtils.cmn().toNullIfEmpty(f.getInput_where());
        if (where != null) {
            sqlMin += " WHERE " + f.getInput_where();
            sqlMax += " WHERE " + f.getInput_where();
        }
        TS_SQLConnWalkUtils.query(anchor,
                sqlMin,
                fillStmt -> {
                },
                rs -> f.setOutput_minId(rs.lng.get(0, 0))
        );
        d.ci("run", "min", f.getOutput_minId());
        TS_SQLConnWalkUtils.query(anchor,
                sqlMax,
                fillStmt -> {
                },
                rs -> f.setOutput_maxId(rs.lng.get(0, 0))
        );
        d.ci("run", "max", f.getOutput_maxId());
    }
}
