...
public class AppSGECellUpdateSTR extends TS_SGWTExecutor {

    final private static TS_Log d = TS_Log.of(AppSGECellUpdateSTR.class);
    ...
    public TS_SGWTValidationResult validate(TS_ThreadSyncTrigger servletKillTrigger, HttpServletRequest rq, TGS_SGWTFuncBase funcBase) {
        var f = (AppSGFCellUpdateSTR) funcBase;
    ...
    public void run(TS_ThreadSyncTrigger servletKillTrigger, HttpServletRequest rq, TGS_SGWTFuncBase funcBase, Object vldRtn) {
        ...
        if (!f.getInput_dbCfg().isAny()) {
            ...
            if (cr.getColType().typeStrLnk()) {//IF STR LINK
                List<Long> curValsAsLng = new ArrayList();
                {//TOKENS SHOULD BE CAST TO LONG
                    var curVals = TGS_StringUtils.jre().toList_spc(curVal).stream().map(s -> s.trim()).toList();
                    for (var curValItem : curVals) {
                        var curValItemLng = TGS_CastUtils.toLong(curValItem).orElse(null);
                        if (curValItemLng == null) {
                            d.ce("ERROR: " + f.getClass().getSimpleName() + " -> curValItemInt == null for " + curValItem);
                            f.setOutput_result(false);
                            return;
                        }
                        curValsAsLng.add(curValItemLng);
                    }
                }
                for (var valLng : curValsAsLng) {//TOKENS SHOULD EXISTS
                    var exists = TS_SQLExistsUtils.exists(anchorConverted, cr.getDataString1_LnkTargetTableName())
                            .whereFirstColumnAsId(valLng);
                    if (!exists) {
                        d.ce("ERROR: " + f.getClass().getSimpleName() + " -> " + cr.getDataString1_LnkTargetTableName() + " -> id: " + valLng + " @" + curVal + " not exists!");
                        f.setOutput_result(false);
                        return;
                    }
                }
        ...
        if (!f.getInput_dbCfg().isAny()) {
            ...
            if (u_add.isExcuse()) {
                d.ce("ERROR: ", f.getClass().getSimpleName(), AppSGFCellUpdateSTR.class.getSimpleName() + ".backup == false", f, u_add.excuse().getMessage());
                return;
            }
        }
