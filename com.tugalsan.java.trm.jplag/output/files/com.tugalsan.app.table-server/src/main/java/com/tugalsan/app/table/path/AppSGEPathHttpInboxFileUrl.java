...
public class AppSGEPathHttpInboxFileUrl extends TS_SGWTExecutor {

    final private static TS_Log d = TS_Log.of(AppSGEPathHttpInboxFileUrl.class);
    ...
    public TS_SGWTValidationResult validate(TS_ThreadSyncTrigger servletKillTrigger, HttpServletRequest rq, TGS_SGWTFuncBase funcBase) {
        var f = (AppSGFPathHttpInboxFileUrl) funcBase;
...
//        var loginCard = (TGS_LibLoginCard) vldRtn;
        ...
        var url = f.getInput_url();
        d.ci("run", "tablename", tablename);
        d.ci("run", "columnname", columnname);
        d.ci("run", "filename", filename);
        d.ci("run", "url", url);

        var urlOut = TS_LibTableFileGetUtils.urlTblFile(
                cp.dirDAT, TGS_Url.of(url),
                tablename, columnname, filename
        );
        d.ci("run", "urlOut", urlOut);
        f.setOutput_url(urlOut.toString());
    }
}
