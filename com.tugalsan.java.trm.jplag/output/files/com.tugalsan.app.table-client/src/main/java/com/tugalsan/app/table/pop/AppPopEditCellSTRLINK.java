...
import com.tugalsan.lib.rql.link.client.TGS_LibRqlLink;
...
public class AppPopEditCellSTRLINK implements TGC_PopInterface {

    final private static TGC_Log d = TGC_Log.of(AppPopEditCellSTRLINK.class);
    private TextBox tbInput;
    private TGS_LibRqlTbl tarTable;
    private AppCell_STRLINK cell;
    ...
    public void createWidgets() {
        ...
        btnSelectAllSearch = TGC_ButtonUtils.createIcon(TGS_IconUtils.CLASS_CHECKBOX_CHECKED());
        btnSelectNoneSearch = TGC_ButtonUtils.createIcon(TGS_IconUtils.CLASS_CHECKBOX_UNCHECKED());
        btnSelectAllValues = TGC_ButtonUtils.createIcon(TGS_IconUtils.CLASS_CHECKBOX_CHECKED());
        btnSelectNoneValues = TGC_ButtonUtils.createIcon(TGS_IconUtils.CLASS_CHECKBOX_UNCHECKED());
        btnNone = TGC_ButtonUtils.createIcon(TGS_IconUtils.CLASS_CHECKBOX_UNCHECKED(), "Listeyi Boşalt");
        btnAll = TGC_ButtonUtils.createIcon(TGS_IconUtils.CLASS_CHECKBOX_CHECKED(), "Liste Hepsi");
        ...
        cbDistinct = TGC_CheckBoxUtils.createIcon(TGS_IconUtils.CLASS_HIPSTER(), " Varsa ekleme");
    ...
    private PushButton btnSearch, btnCancel, btnSave, btnSearchAll, btnTableTargetOpen, btnMinMax;
    private PushButton btnSelectAllSearch, btnSelectNoneSearch, btnSelectAllValues, btnSelectNoneValues;
    private PushButton btnNone, btnAll, btnAdd, btnRemove, btnLinkMemoryEnabled;
    private TextBox tbInputMin, tbInputMax;
    private CheckBox cbDistinct;
    ...
    public void configInit() {
        ...
        cbDistinct.setValue(true);
        values = TGS_ListUtils.of();
        lbValues.setStyleName(TGC_PanelBlue.class.getSimpleName());
    ...
    public void configActions() {
        ...
        TGC_ClickUtils.add(btnAll, onAll);
        ...
        TGC_KeyUtils.add(btnAll, onAll, onCancel);
        ...
        TGC_KeyUtils.addCtrlEnter(onSetCell,
                ...
                btnSelectNoneSearch, btnSelectNoneValues, btnNone, btnAll, btnAdd, btnRemove, lb, lbValues, tbInput
        );
    }
    ...
    public void configFocus() {
        ...
        TGC_FocusUtils.addKeyUp(lb, new TGS_FocusSides4(null, cbDistinct, tbInput, btnSearchAll), null);
        TGC_FocusUtils.addKeyUp(btnSearchAll, new TGS_FocusSides4(null, btnTableTargetOpen, lb, null));
        TGC_FocusUtils.addKeyUp(btnTableTargetOpen, new TGS_FocusSides4(btnSearchAll, btnAll, lb, null));

        TGC_FocusUtils.addKeyUp(lbValues, new TGS_FocusSides4(cbDistinct, null, null, null));
                ...
                switch (nativeKeyCode) {
                    ...
                    case KeyCodes.KEY_DOWN:
                        if (lb.getSelectedIndex() == lb.getItemCount() - 1) {
                            TGC_FocusUtils.setFocusAfterGUIUpdate(btnAll);
                        }
                        break;
                    default:
                        break;
        ...
        TGC_FocusUtils.addKeyUp(btnAll, new TGS_FocusSides4(btnTableTargetOpen, btnNone, lbValues, null));
        TGC_FocusUtils.addKeyUp(btnNone, new TGS_FocusSides4(btnAll, null, lbValues, null));

        TGC_FocusUtils.addKeyUp(cbDistinct, new TGS_FocusSides4(lb, lbValues, null, btnAdd));
        TGC_FocusUtils.addKeyUp(cbDistinct, nativeKeyCode -> {
            if (null != nativeKeyCode) {
                switch (nativeKeyCode) {
                    case KeyCodes.KEY_UP:
                        TGC_FocusUtils.setFocusAfterGUIUpdate(tbInput);
        ...
        TGC_FocusUtils.addKeyUp(btnAdd, new TGS_FocusSides4(lb, lbValues, cbDistinct, btnRemove));
    ...
    public void configLayout() {
        ...
        TGC_PanelAbsoluteUtils.setWidget(content, btnAll, TGS_ShapeRectangle.of(580, 450, 195, 24));
    ...
    public AppPopEditCellSTRLINK(AppModuleTable tm) {
        this.tm = tm;
        createWidgets();
        createPops();
        configInit();
    ...
    private TGS_FuncMTU onTableLinkOpen = () -> {
        ...
        if (!tm.subRecord.onClick_TableLinkOpen(cell)) {
    ...
    private TGS_FuncMTU onAdd = () -> {
        TGC_ThreadUtils.run_afterGUIUpdate(() -> {
            ...
            if (si.isEmpty()) {
                ...
                d.ce("HATA: Eklemeden önce sol taraftaki listeden birkaç satır seçiniz.");
                return;
            }
            si.forEach(idx -> {
                var text = lb.getItemText(idx);
                var parsedRow = TGS_StringUtils.gwt().toList_spc(text);
                if (parsedRow.isEmpty()) {
                    TGC_PanelStyleUtils.warn(lb, 1);
                    d.ce("HATA: Solda seçilen satırı parçalama hatası.");
                    return;
                }
                var id = TGS_CastUtils.toInteger(parsedRow.get(0)).orElse(null);
                if (id == null) {
                    TGC_PanelStyleUtils.warn(lb, 1);
                    d.ce("HATA: Solda seçilen satırda ilk kelime id olmalıydı hatası.");
                    return;
                }
                ...
                if (cbDistinct.getValue() && values.stream().anyMatch(v -> v.equals(ids))) {
                    TGC_PanelStyleUtils.warn(lbValues, 1);
                    d.ce("UYARI: Zaten var!");
                }
                values.add(ids);
    ...
    private TGS_FuncMTU onRemove = () -> {
        TGC_ThreadUtils.run_afterGUIUpdate(() -> {
            ...
            if (si.isEmpty()) {
                TGC_PanelStyleUtils.warn(lbValues, 1);
                d.ce("HATA: Kaldırmadan önce sağ taraftaki listeden birkaç satır seçiniz.");
                return;
            }
    ...
    private TGS_FuncMTU onAll = () -> {
        onSetValuesNone.run();
        values.add(TGS_LibRqlLink.STAR_TAG());
        lbValues.addItem("* HEPSİ");
    };
...
//                }, 1);
                ...
                resp.getOutput_list().forEach(s -> {
                    lb.addItem(s);
                    d.ci("refreshLb", "callBack", "r.getDataArrayString(i)", s);
                });
    ...
    public TGS_FuncMTU_In2<Boolean, String> onSetVisible = (enable, lblHtmlString) -> {
        if (enable) {
            cell = (AppCell_STRLINK) tm.cells.getActiveCell();
            ...
            tbInput.setText(cell.getText());

            values.clear();
            lbValues.clear();
            values = TGS_StringUtils.gwt().toList_spc(cell.getText());

            List<IDX_ID> lst_IDX_ID = new ArrayList();
            IntStream.range(0, values.size()).forEachOrdered(idx -> {
                var id = TGS_CastUtils.toInteger(values.get(idx)).orElse(null);
                if (id == null) {
                    if (values.get(idx).isEmpty()) {
                        ...
                        d.ce("setVisible", "ERROR: @ values.get", idx, values.get(idx), "nor integer and nor '*'");
                    }
                } else {
    ...
    private final TGS_FuncMTU onCancel = () -> {
        onSetVisible.run(false, null);
    };

    private final TGS_FuncMTU onSetCell = () -> {
        var valueString = TGS_StringUtils.cmn().toString(values, " ");
        ...
        TGC_SGWTCalller.async(new AppSGFCellUpdateSTR(tm.dbCfg, tm.curTable, id, columnname, valueString), r -> {
            if (r.getOutput_result()) {
                d.cr("onSetCell", "Değişiklik yapıldı.");
                cell.setValueString(valueString);
            } else {
                d.ce("onSetCell", "HATA: bağlantı değişirken hata oluştu! (TK_GSF_SQLCellUpdateSTR)");
            }
            TGC_FocusUtils.setFocusAfterGUIUpdate(cell);
            onSetVisible.run(false, null);
        });
